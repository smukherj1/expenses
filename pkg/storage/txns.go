...
// * For each source and globally:
//   * Total tagged & untagged credits vs total credits
//   * Total untagged & untagged debits vs total debits
//   * Top 10 tags by txn count
//   * Top 10 tags by credits
//   * Top 10 tags by debits

type TxnAmounts struct {
	Credits int64 `json:"credits,omitempty"`
	Debits  int64 `json:"debits,omitempty"`
}

type TxnTagMetric struct {
...
type TxnOverview struct {
	...
	TaggedAmounts   TxnAmounts `json:"tagged_amounts,omitempty"`
	UntaggedAmounts TxnAmounts `json:"untagged_amounts,omitempty"`

	TopTagsByCount   []TxnTagMetric `json:"top_tags_by_count,omitempty"`
	TopTagsByCredits []TxnTagMetric `json:"top_tags_by_credits,omitempty"`
	TopTagsByDebits  []TxnTagMetric `json:"top_tags_by_debits,omitempty"`
}

func (s *Storage) TxnsOverviews(ctx context.Context) ([]TxnOverview, error) {
	bySrc := make(map[string]TxnOverview)
	q := `
WITH CategorizedTxns AS (
  SELECT
...
GROUP BY SOURCE, TAGGED;
`
	rows, err := s.db.QueryContext(ctx, q)
	if err != nil {
		return nil, fmt.Errorf("unable to query transactions categorized by source from database: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var src string
		var tagged bool
		var count, credits, debits int64
		if err := rows.Scan(&src, &tagged, &count, &credits, &debits); err != nil {
			return nil, fmt.Errorf("unable to scan row from transactions categorized by source fetched from database: %w", err)
		}
		to, ok := bySrc[src]
		if !ok {
			to = TxnOverview{
		...
		if tagged {
			to.TaggedAmounts = TxnAmounts{
				Credits: credits,
				Debits:  debits,
			}
		} else {
			to.UntaggedAmounts = TxnAmounts{
				Credits: credits,
				Debits:  debits,
			}
		}
	...
	var result []TxnOverview
	for _, to := range bySrc {
...
WITH CategorizedTxns AS (
  SELECT
  CASE WHEN AMOUNT_CENTS >= 0 THEN AMOUNT_CENTS ELSE 0 END AS credit_amount,
  CASE WHEN AMOUNT_CENTS < 0 THEN -AMOUNT_CENTS ELSE 0 END AS debit_amount,
  CASE WHEN CARDINALITY(TAGS) > 0 THEN 'tagged' ELSE 'untagged' END AS tagged,
  source
  FROM Transactions
)

SELECT source, tagged, COUNT(*) AS count, SUM(credit_amount) AS credits, SUM(debit_amount) AS debits FROM CategorizedTxns GROUP BY source, tagged;
*/
